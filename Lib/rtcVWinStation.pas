...
// Define to use a fix for the MS RDP problem
{$DEFINE RTC_LBFIX}
...
function SwitchToActiveDesktop(forced:boolean):boolean;
...
function SelectInputWinStation:boolean;
var
  flags: USEROBJECTFLAGS;
  tmp: Cardinal;
  oldstat:HWINSTA;
begin
  oldstat:=home_window_station;
  ...
  Result:=home_window_station<>oldstat;
end;
...
begin
  ...
  try
    ...
    if not forced then
      forced:=not SwitchDesktop(CurDesktop);
    if forced then
      begin
      LogonDesktop := OpenInputDesktop(DF_ALLOWOTHERACCOUNTHOOK, True, DESKTOP_ALL);
